...
export default function Header() {
    ...
    return (
        <header className="relative w-full" style={{ height: '160px' }}>
            ...
            <div 
                id="navigation" 
                className={`bg-[rgba(255,255,255,0.95)] h-[80px] flex justify-center
                    ${isSticky ? 'fixed top-0 left-0 right-0 z-50 shadow-md transition-all duration-300' : ''}`}
            >
                ...
                <div className="justify-start flex items-center">
                    <ul className="flex justify-start space-x-4 text-black items-center font-commissioner">
                        <li className={catalogButton} onMouseEnter={() => setShowCatalog(true)} onMouseLeave={() => setShowCatalog(false)}>
                ...
                {showCatalog && (
                                <div className="absolute left-1/2 transform -translate-x-1/2 w-[500px] h-[500px] bg-white border border-PLGreen shadow-lg z-50" 
                                     style={{top: '100%'}}>
                                    {/* Содержимое каталога */}
                                </div>
                            )}
